...
from services.task_service import create_task, list_tasks, seed_demo_tasks
...
from utils.ui.mobile_stitch import admin_candidate_card, admin_mobile_bento, admin_mobile_header
...
with st.container(key="admin_page"):
    ...
    with tab_candidates:
        ...
        for idx, candidate in enumerate(pending):
            ...
            score = int(screening.get("score", 0)) if screening else None
            skills = candidate.get("skills", [])[:2]
            admin_candidate_card(
                candidate["name"],
                f"{format_comma_list(skills) or 'Applicant'} · {candidate.get('email', '')}",
                score=score,
                tags=skills,
                highlighted=idx == 0,
            )
            with st.expander(f"Review {candidate['name']}", expanded=idx == 0):
        ...
        open_tasks = [t for t in list_tasks() if t.get("status") == "open"]
        admin_mobile_bento(open_tasks=len(open_tasks), assignable=len(volunteers))

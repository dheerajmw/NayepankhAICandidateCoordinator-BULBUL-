...
from utils.ui.mobile_stitch import task_mobile_match_card, task_mobile_status_header
...
with st.container(key="tasks_page"):
    kpi_row(
        [
    ...
    with tab_ai:
        ...
        if matches:
            total_matches = sum(len(item["matches"]) for item in matches)
            task_mobile_status_header(accuracy=92, matches=total_matches)
        if not matches:
            ...
            for item in matches:
                ...
                for idx, match in enumerate(item["matches"]):
                    task_mobile_match_card(
                        match.get("volunteer_name", "Volunteer"),
                        match.get("priority", "Volunteer"),
                        task["title"],
                        match.get("reasoning", ""),
